...
# --- 修正後的 CSS：移除固定背景色，改用透明與陰影 ---
st.markdown("""
    <style>
    /* 讓卡片在深色模式下也能辨識，使用 rgba 控制透明度 */
    .stMetric { 
    ...
    .stButton>button:hover { background-color: #e64a19; color: white !important; }
...
with st.expander("📝 點名紀錄提交", expanded=True):
    ...
    if st.button("🚀 提交紀錄"):
        if name:
            ...
            st.success(f"✅ 紀錄成功！(點名者：{current_user})")
            st.rerun()
