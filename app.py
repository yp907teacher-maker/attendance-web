...
with st.sidebar:
    ...
    current_user = st.selectbox("🙋 點名教練", staff_list)
    st.divider()
...
if not raw_df.empty:
    ...
    # 統計全歷史數據 (用於 10 堂制與私假計次)
    total_stats = calc_df.groupby('學生姓名').apply(lambda x: pd.Series({
...
if not display_df.empty:
    ...
    # 重新排列欄位順序更易讀
    cols = ['班別', '學生姓名', '月出席', '月缺席', '月補課', '剩餘堂數(10堂制)']
    st.dataframe(report_df[[c for c in cols if c in report_df.columns]], use_container_width=True, hide_index=True)
...
if search_name:
    ...
    with col_b:
        st.write("📊 **私假與堂數統計**")
        # 使用 str.contains 增加搜尋彈性
        private_leave_count = len(p_df[(p_df['狀態'] == '缺席') & (p_df['假別備註'].str.contains('私假', na=False))])
        if private_leave_count >= 1:
            st.error(f"該生本期已請私假 {private_leave_count} 次 (制度規定僅限補課 1 次)")
        else:
            st.success("該生本期尚未請過私假")
        
        last_mode = p_df['收費模式'].iloc[-1] if not p_df.empty else "未知"
        st.info(f"當前收費模式：{last_mode}")
